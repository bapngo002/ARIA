...
try {
  ...
  function front(){camera.position.set(0,1.46,.62);controls.target.set(0,1.46,0);controls.update();}
  front();document.getElementById('front').onclick=front;
  document.getElementById('quarter').onclick=()=>{camera.position.set(.31,1.46,.54);controls.target.set(0,1.46,0);controls.update();};
  scene.add(new T.HemisphereLight(0xfff2e6,0x6e577b,.85));
  const light=new T.DirectionalLight(0xffe2c9,2.2);light.position.set(-2,3,4);scene.add(light);
  const fill=new T.DirectionalLight(0xcbd8ff,.45);fill.position.set(2,1,2);scene.add(fill);
  const rim=new T.DirectionalLight(0xe8d1ff,1.1);rim.position.set(2,3,-2);scene.add(rim);
...
} catch(error){loading.textContent='Không mở được bản 3D. Bạn có thể tải file GLB bên dưới để xem trong Blender.';}
