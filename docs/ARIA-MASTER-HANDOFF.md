...
Verified: local Blender saves and multiview renders through R17; R16/R17 oblique/profile inspection exposed photo stretching at the temple, incomplete hair coverage and an inaccurate lower-face silhouette. R18 inspection still showed hard skin-projection borders. R19-R20 rebuilt continuous hair coverage; R21 corrected lip/chin depth using sampled reference profile silhouette and smoothed lateral surfaces. R22 per-pixel hair masking alone did not resolve visible skin borders. A clay diagnostic verified a continuous, smooth head surface; isolated textured-head rendering traced the remaining jagged appearance to skin projection boundaries. R23-R25 extended clean skin-edge colors and rebuilt normalized per-pixel hair projection; inspected renders still show conspicuous temporal/ear seams and view-dependent artifacts. R26 restricts photographic detail to the central face with a smooth side blend; front, three-quarter and near-profile renders visually inspected. Visible hairline blocks, exposed ear/temple patches and profile mismatch remain; R26 does NOT meet the requested likeness. Appearance remains below the owner requested standard. Repeated projection adjustments have not established a satisfactory three-view likeness; do not treat a frontal resemblance as completion. Actual head, eyes and curved hair meshes exist. Appearance is heavily dependent on low-resolution source-photo projection, not a finished reconstructed texture or strand groom. Likeness remains INCOMPLETE; do not call these animation-ready. No rig/teeth/blendshape/export validation for these appearance studies; R6 export checks do not transfer.

Inputs and provenance: assets/characters/aria-custom/reference-fit-inputs/README.md and saved JSON/NPY inputs. Local MediaPipe landmarks guide 142 frontal constraints; this 2D fit residual is NOT 3D accuracy or likeness evidence. Builders fit_head_r8.py and refine_r9.py through refine_r26.py retain the actual mathematical edits and local dependencies. Original three-view reference is packed in the Blender files. Next: resolve hair/temple material boundaries and assess real three-view likeness rather than continuing blind projection patches, then reintroduce facial animation only after appearance is satisfactory. Usage at 4% remaining prompted this recovery checkpoint; task is not declared complete.
...
### Custom head R2 — static eyes and profile, 2026-09-13

Created `assets/characters/aria-custom/aria-head-r2.blend` from preserved R1. Replaced temporary clay eye spheres with ivory sclera and curved brown iris/pupil geometry/materials at MPFB eye landmarks; applied five editable bilateral eyelid-height/nose-tip/upper-lip targets. VERIFIED: Blender build/save and front/three-quarter/profile renders; front and oblique render inspected. PARTIALLY VERIFIED: static eye fitting only, not motion/contact across expressions. NOT VERIFIED/pending: exact concept likeness, tearline, brows/lashes, hair, skin detail, gaze/blink rig, expressions and Pi deployment. Builder/settings retained in refine_head_r2.py/head-r2-report.json; launcher opens R2. Browser remains old rejected avatar, not this new Blender work. Next modeling step: refine eyelid margins/tearline and brows, then reassess face likeness before hair and animation. Hardware holds unchanged.

### Custom head proportion study R1 — 2026-09-13

Completed the next modeling pass in `assets/characters/aria-custom/aria-head-blockout-r1.blend`, preserving the original MPFB base. Thirteen editable MPFB shape targets adjust the oval silhouette, jaw/chin, nose width, lip volume/cupid bow/corners and bilateral eye scale. This is a parametric proportion study, not a hand-sculpted or approved likeness. Added temporary clay eye spheres at MPFB eye landmarks and non-destructive subdivision for review. VERIFIED: Blender build/save and clay renders (before front, R1 front/three-quarter/profile), inspected front and profile; source values are in head-r1-report.json. PARTIALLY VERIFIED: proportion direction only, not measured likeness; temporary eye fitting still needs refinement. NOT VERIFIED: exact resemblance, skin, hair, clothing, facial rig/expressions and Pi performance. Launcher now opens R1; browser preview still shows the rejected earlier model and is not evidence for this new Blender work. Next modeling task: refine eyelids/eye fitting and nose/lip profile against reference, then recheck silhouette before hair and rigging. Hardware holds unchanged.
...
**Appearance refinement, following owner request:** revised the editable draft and GLB with smaller eye openings, fuller lip/cheek/jaw volumes, broader nasal bridge, warmer complexion/lip/orbital vertex colors, restrained PBR reflections and fine surface-following hair geometry. Fixed GLB color-layer selection: imported white COLOR_0 had hidden the new complexion COLOR_1; skin now explicitly uses the new layer. Viewer has closer portrait framing, directional fill/rim lighting and a three-quarter button. VERIFIED: updated Blender front/three-quarter renders, local browser front/oblique appearance, no JS errors in checked state; 48,103 vertices, 154 bones, 57 facial shape keys. Source/previous draft preserved in Git (ccef30b). PARTIALLY VERIFIED: existing sample movement with refined mesh, not exhaustive morph verification. NOT VERIFIED: realism or resemblance matching concept 1, Pi performance or integration. Current stock topology remains anime-like; next quality step requires deeper eyelid/nose/lip and hair-lock remodeling, not another claim of completion based on lighting. Main UI and hardware remain unchanged.
...
**VERIFIED:** Blender 5.2 export/render, 25,207 mesh vertices, 154 bones, 57 facial shape keys; GLB loaded/rendered in the local PC browser with two animation clips and advancing frame counters, no reported JS errors in the checked state. Fixed incorrect transparent-material export that previously obscured hair/face; current materials are opaque like the source. Existing 17 host tests and viewer JavaScript syntax checks pass. **PARTIALLY VERIFIED:** exported demonstration head/face animation; not every facial morph was visually checked. Body retains the authored T-pose because VRM sleeve constraints are not fully imported. **NOT VERIFIED:** concept-1 likeness/realistic quality, owner acceptance, full-body acting, audio lip-sync, Pi performance and deployment. Next visual step: review and refine facial volumes/proportions against concept 1 before main-UI integration. All hardware HOLD boundaries remain active.

Design-selection checkpoint (before the draft above): owner requests image concepts and has no character model file. Generated four standalone portrait concepts with built-in image_gen, saved under `docs/design/aria-face-concepts/` with exact prompts. VERIFIED: four PNG previews generated and preserved on D. They are static design images, NOT 3D models or evidence that this quality runs on Pi. Owner subsequently selects concept 1 ("mẫu 1 hợp nhất"): `docs/design/aria-face-concepts/01-long-hair.png` is the approved visual direction: long dark hair, warm brown eyes, soft facial volumes, restrained smile and indigo clothing. VERIFIED owner design choice; this does not approve the earlier stock VRM or the new draft.
