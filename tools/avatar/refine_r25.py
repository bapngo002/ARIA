...
root=Path(__file__).resolve().parents[2];out=root/'assets/characters/aria-custom/r25-review';out.mkdir(parents=True,exist_ok=True)
bpy.ops.wm.open_mainfile(filepath=str(root/'assets/characters/aria-custom/r24-projection/aria-head-r24.blend'))
h=bpy.data.objects['ARIA R23 - face'];h.name='ARIA R25 - face';m=h.data.materials[0];n=m.node_tree.nodes;l=m.node_tree.links
...
for py in range(396):
 ...
 if len(valid)>5:
  ...
  if py<195:left=np.array([.78,.60,.55]);right=np.array([.80,.62,.56])
...
tex=next(x for x in n if x.type=='TEX_IMAGE' and x.inputs['Vector'].links and getattr(x.inputs['Vector'].links[0].from_node,'uv_map','')=='ReferenceFrontProjection' and x.image.name.startswith('Expanded clean face projection'));tex.image=im
...
camera((0,-1.2,0));bpy.context.preferences.filepaths.save_version=0;bpy.ops.file.pack_all();bpy.ops.wm.save_as_mainfile(filepath=str(out/'aria-head-r25.blend'))
